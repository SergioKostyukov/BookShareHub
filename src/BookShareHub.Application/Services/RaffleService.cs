...
{
	internal class RaffleService(ILogger<RaffleService> logger,
								  BookShareHubDbContext context,
								  IMapper mapper) : IRaffleService
	{
		private readonly ILogger<RaffleService> _logger = logger;
		...
		public async Task<List<RaffleTitleDto>> GetAllRafflesByFilterAsync(RaffleFilter filter, string userId)
		{
